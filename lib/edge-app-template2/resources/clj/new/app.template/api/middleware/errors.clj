(ns {{root-ns}}.api.middleware.errors
...
(defn title-case
  "Turns something like `:subscription_active-paid-subscription`
  into \"Subscription: Active Paid Subscription\""
  [s]
  (-> s
      name
      (str/replace "_" ":-")
...
(def {{root-ns}}-errors
  {:name ::{{root-ns}}-errors
   :wrap (fn [handler error-registry & [overrides]]
           (let [mw (exception/create-exception-middleware (merge (default-handlers error-registry)
                                                                  (or overrides {})))]
             ((:wrap mw) handler)))})
